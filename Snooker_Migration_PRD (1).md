...
RULE 13 — UI VISIBILITY & CONTRAST (CRITICAL)
  Dark theme does NOT mean everything is the same dark color.
  Every element must be CLEARLY visible and distinct.
  Apply the contrast rules defined in Section 13 of this PRD.
  No card, ball, button, or text must blend into the background.
  If two adjacent elements have similar darkness — add a visible
  border, a lighter surface color, or a glow/shadow to separate them.
```
...
13. [UI Visibility & Contrast Rules](#13-ui-visibility--contrast-rules)
...
## 13. UI Visibility & Contrast Rules

Dark theme looks bad when everything blends into the same darkness.
Every element must pop. Follow these rules for every screen.

---

### 13.1 Surface Hierarchy — Three Distinct Levels

Never place same-color surfaces next to each other.
Always use this 3-level system:

```
Level 1 — Page background:   AppColors.bgPage     (#080F1E)  ← deepest
Level 2 — Cards / rows:      AppColors.bgCard     (#0E1A2E)  ← mid
Level 3 — Elevated / chips:  AppColors.bgElevated (#162236)  ← lightest dark
```

Rule: A card on bgPage must be bgCard. A chip inside a card must be bgElevated.
Never put a bgCard element on a bgCard background — they merge invisibly.

---

### 13.2 Card Borders — Always Visible

Every Card, Container acting as a card, and list row MUST have a visible border:

```dart
decoration: BoxDecoration(
  color: AppColors.bgCard,
  borderRadius: BorderRadius.circular(16),
  border: Border.all(color: AppColors.border, width: 1),  // REQUIRED
)
```

`AppColors.border` is `#1A2A44` — this is a subtle but visible blue-tinted border
that separates cards from the dark background. Never skip this.

For elevated/active items add a blue glow instead of just a border:
```dart
boxShadow: [
  BoxShadow(
...
### 13.3 Snooker Balls — Each Ball Must Be Clearly Visible

The _BallGrid is the most important UI element. Each ball must look like
a distinct, real snooker ball — not a flat dark circle.

**Required for every ball button:**

```dart
// Outer container — always add a colored ring border
Container(
  decoration: BoxDecoration(
    shape: BoxShape.circle,
    border: Border.all(
      color: ballColor.withValues(alpha: 0.6),  // colored ring
      width: 2.5,
    ),
    boxShadow: [
      BoxShadow(
        color: ballColor.withValues(alpha: 0.45),  // colored glow
        blurRadius: 14,
        spreadRadius: 1,
      ),
    ],
  ),
  child: // the 3D sphere widget
)
```

**Black ball special treatment:**
The black ball (#1F2937) is almost invisible on dark backgrounds.
Replace its base color with a lighter charcoal for the button:
```dart
// Black ball button surface color:
Color(0xFF374151)  // visible charcoal instead of near-black
// Black ball border ring:
Color(0xFF9CA3AF)  // light grey ring to define the shape
// Black ball glow:
Color(0xFF6B7280).withValues(alpha: 0.5)
```

**Ball label text — always readable:**
```
Yellow ball  → text color: Color(0xFF78350F)  (dark amber — readable on gold)
Green ball   → text color: Colors.white
Brown ball   → text color: Colors.white
Blue ball    → text color: Colors.white
Pink ball    → text color: Colors.white
Black ball   → text color: Colors.white
Red ball     → text color: Colors.white
```

---

### 13.4 Player List Items — Active vs Others Must Be Obvious

**Active player row:**
```dart
// Background: slightly lighter than bgCard to stand out
color: AppColors.bgElevated,
// Left accent bar: 4px wide, full height, AppColors.primary (blue)
// Score text: AppColors.accent (cyan) — bright, stands out
// Row border: Border.all(color: AppColors.primary.withValues(alpha:0.4), width:1)
```

**Completed player row:**
```dart
// Background: AppColors.bgCard with warning tint
color: AppColors.warningBg,  // subtle gold background
// Left accent bar: AppColors.warning (gold)
// Score: AppColors.warning with ⭐ icon
// Row border: Border.all(color: AppColors.warning.withValues(alpha:0.3), width:1)
```

**Inactive player row:**
```dart
// Background: AppColors.bgCard
// Left accent bar: AppColors.border (subtle)
// Score: AppColors.textSecondary
// Row border: Border.all(color: AppColors.border, width:1)
```

All three states must look clearly different from each other.

---

### 13.5 Current Player Card — Must Feel Premium

The _CurrentPlayerCard is the hero element of the game screen.
It must look bright, premium, and immediately catch the eye.

```dart
decoration: BoxDecoration(
  gradient: LinearGradient(
    colors: [Color(0xFF0D2D6B), Color(0xFF071A40)],  // brighter blue gradient
    begin: Alignment.topLeft, end: Alignment.bottomRight,
  ),
  borderRadius: BorderRadius.circular(20),
  border: Border.all(
    color: AppColors.primary.withValues(alpha: 0.5),  // blue border
    width: 1.5,
  ),
  boxShadow: [
    BoxShadow(
...
"NOW PLAYING" label:
```dart
// Pill background: AppColors.accent.withValues(alpha: 0.15)
// Pill border: AppColors.accent.withValues(alpha: 0.4)
// Text: AppColors.accent  (bright cyan)
// Letter spacing: 2px  (uppercase)
```

Score number:
```dart
// Color: Colors.white  (pure white — max contrast)
// Font size: 64, Syne, weight 800
// Add subtle text shadow: AppColors.primary, blur 20
```

---

### 13.6 Buttons — Never Blend Into Background

**Primary gradient button (Next Player):**
```dart
// Gradient: AppColors.primary → AppColors.accent
// Add border: AppColors.accent.withValues(alpha: 0.4), width 1
// Add glow shadow: AppColors.primary.withValues(alpha: 0.4), blur 16
// Text: Colors.white, fontWeight: w700
```

**Outlined button (Undo):**
```dart
// Background: AppColors.bgElevated  (NOT transparent — too invisible)
// Border: AppColors.border, width 1.5
// Text/icon: AppColors.textSecondary
// On hover/tap: border becomes AppColors.primary
```
...
// Size: 46×46 circle
// Icon: white
// Box shadow: AppColors.primary.withValues(alpha:0.45), blur 12
```

---

### 13.7 Subtract Mode Toggle — Must Scream "WARNING"

When subtract mode is ON, the whole toggle area must be unmistakably red.

```dart
// ON state:
// Background gradient: #EF4444 → #DC2626
// Border: Color(0xFFFF6B6B).withValues(alpha:0.6), width 1.5
// Icon + text: Colors.white
// Box shadow: Color(0xFFEF4444).withValues(alpha:0.4), blur 16

// OFF state:
// Background: AppColors.bgCard
// Border: AppColors.border, width 1.5
// Icon + text: AppColors.textMuted
```

---

### 13.8 History Rows — Ball Color Must Be Obvious

Each history row has a colored left border matching the ball.
Make this border thicker and more visible:

```dart
// Left border width: 4px (not 1–2px)
// Ball emoji size: 20px
// Each row: alternate between bgCard and bgElevated for zebra readability
//   even index: bgCard
//   odd index:  bgElevated
```

Score badges:
```dart
// +X badge: background successBg, border success color (1px), text success
// −X badge: background dangerBg, border danger color (1px), text danger
...
### 13.9 Settings Screen — Clear Group Separation

Each settings group card needs clear visual separation:
...
//   color: AppColors.accent  (NOT textMuted — must be visible)
//   font size: 10px, letter spacing: 2px, uppercase

// Row divider inside groups:
//   color: AppColors.border, height: 1px
//   indent: 16px left (don't go edge to edge)

// Target score pills:
//   Selected: background primary, white text, border primary
//   Unselected: background bgElevated, textMuted text, border border
//   All pills: border radius 20px, padding 8×16px
```

---

### 13.10 Draw Screen — Reveal Card Must Pop

The _RevealCard showing the drawn name is the key moment of the Draw screen.
It must be highly visible:

```dart
// Background gradient: Color(0xFF0D2D6B) → Color(0xFF071A40)
// Border: AppColors.accent.withValues(alpha:0.5), width 1.5
// Box shadow: AppColors.accent.withValues(alpha:0.2), blur 24, spread 2

// Drawn name text:
//   color: Colors.white  (pure white, not textPrimary)
//   font size: 42px, Syne, weight 800
//   add text shadow: AppColors.accent, blur 20

// Position badge (#N):
//   background: AppColors.primary
//   border: AppColors.accent.withValues(alpha:0.4), width 1
//   text: white, Syne, weight 700
```

Candidate name chips (before draw):
```dart
// Background: AppColors.bgElevated  (NOT bgCard — too dark)
// Border: AppColors.border, width 1
// Text: AppColors.textPrimary
// Remove icon: AppColors.textMuted
```

---

### 13.11 Bottom Navigation Bar — Active Tab Obvious

```dart
BottomNavigationBar(
  backgroundColor: AppColors.navbar,
  selectedItemColor: AppColors.accent,        // bright cyan
  unselectedItemColor: AppColors.textMuted,   // dim
  selectedLabelStyle: TextStyle(fontWeight: FontWeight.w700, fontSize: 11),
  unselectedLabelStyle: TextStyle(fontSize: 11),
  // Add top border to separate from content:
  // Wrap in DecoratedBox with Border(top: BorderSide(color: AppColors.border))
)
```

---

### 13.12 Summary — Contrast Checklist

Before finishing each screen, verify:

```
□ Page background is bgPage (#080F1E)
□ Every card has bgCard background + AppColors.border border
□ Every chip/badge inside a card uses bgElevated background
□ Active player row is visually distinct from inactive rows
□ Completed player row is visually distinct from active rows
□ Current player card has blue gradient + blue glow shadow
□ Each snooker ball has colored ring + colored glow
□ Black ball has light grey ring (not invisible)
□ Primary buttons have glow shadow
□ Outlined buttons have bgElevated background (not transparent)
□ Subtract toggle is unmistakably red when ON
□ Section headers in settings use AppColors.accent (not textMuted)
□ History row left borders are 4px thick
□ Score badges are pill-shaped with matching colored border
□ Draw reveal card has blue gradient + cyan glow
```
...
| 1.1 | Added Section 13 — UI Visibility & Contrast Rules for all elements |
